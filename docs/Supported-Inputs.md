...
- Manufacturer URL: [Avia Semiconductor](https://www.aviaic.com/)
- Datasheet URL: [HX711 Datasheet](https://cdn.sparkfun.com/datasheets/Sensors/ForceFlex/hx711_english.pdf)
- Product URL: [HX711 listings](https://www.amazon.com/s?k=hx711)

Use this input for broad Raspberry Pi compatibility (including Raspberry Pi 5). The HX711 measures tiny bridge changes from load cells and outputs a raw value, which is converted to grams by applying the calibration factor. Advanced calibration runs: mycodo/inputs/scripts/hx711_test.py.

<table><thead><tr class="header"><th>Section</th><th>Notes</th></tr></thead><tbody>
<tr><td>Recommended ranges</td><td>This setup has been validated for heavier ranges (e.g., 1–200 kg). Calibrate near your target range (e.g., 25–40 kg if that is your target).<br>Lower ranges (e.g., 1–20 kg or 0.01–1 kg) have not been tested in this setup yet. If you use a different range, ensure the load cell and amplifier are rated for it and recalibrate accordingly.</td></tr>
<tr><td>Calibration findings</td><td>Use a rigid, flat load plate to distribute weight evenly. Point loads (wheels/feet) and flexible surfaces increase drift and variability.<br>Reposition the weight between calibration runs and average multiple runs to reduce placement bias.<br>Disable zero tracking during calibration to avoid auto-adjusting tare mid-run.<br>Ensure the raw value increases when weight is added. If it decreases, check wiring (A+/A- swapped or inverted bridge).<br>Calibrate within your intended measurement range; very light or very heavy loads reduce accuracy.</td></tr>
<tr><td>Hardware notes</td><td>Bathroom scales are a good donor platform: four load cells with a rigid frame. You can replace the onboard ADC with an HX711 and reuse the mechanical structure.<br>If you retrofit a scale, disconnect the original battery pack and power the HX711 from the Raspberry Pi 3.3V rail (do not connect both).<br>Keep the load cells firmly mounted and avoid any shifting between calibrations.</td></tr>
<tr><td>Do</td><td>Calibrate with a known, stable weight in your target range.<br>Keep the load placement consistent and use a flat, rigid plate to distribute weight.<br>Reposition the weight between calibration runs for better averaging.<br>Disable zero tracking during calibration and re-enable afterward if needed.</td></tr>
<tr><td>Don't</td><td>Use point loads (wheels/feet) or flexible surfaces that bend under load.<br>Mix tare and calibration measurements with different placements/orientation.<br>Expect stable factors if raw values drift or the signal is saturated.</td></tr>
</tbody></table>

<table><thead><tr class="header"><th>Option</th><th>Type</th><th>Description</th></tr></thead><tbody><tr><td>Measurements Enabled</td><td>Multi-Select</td><td>The measurements to record</td></tr><tr><td>Period (Seconds)</td><td>Decimal</td><td>The duration between measurements or actions</td></tr><tr><td>Pre Output</td><td>Select</td><td>Turn the selected output on before taking every measurement</td></tr><tr><td>Pre Out Duration (Seconds)</td><td>Decimal</td><td>If a Pre Output is selected, set the duration to turn the Pre Output on for before every measurement is acquired.</td></tr><tr><td>Pre During Measure</td><td>Boolean</td><td>Check to turn the output off after (opposed to before) the measurement is complete</td></tr><tr><td>Data Pin (DT)</td><td>Integer</td><td>The GPIO pin connected to the HX711 data pin (DT/DOUT)</td></tr><tr><td>Clock Pin (SCK)</td><td>Integer</td><td>The GPIO pin connected to the HX711 clock pin (SCK/PD_SCK)</td></tr><tr><td>Channel</td><td>Select</td><td>The HX711 channel to read from (A or B)</td></tr><tr><td>Gain</td><td>Select</td><td>Gain selection (A: 128/64, B: 32)</td></tr><tr><td>Samples</td><td>Integer</td><td>The number of samples to average per measurement</td></tr><tr><td>Outlier Filter Enabled</td><td>Boolean</td><td>Filter spikes using a median/MAD filter before averaging</td></tr><tr><td>Outlier MAD Threshold</td><td>Decimal</td><td>Lower is stricter; typical range 2.5–5.0</td></tr><tr><td>Tare Value</td><td>Decimal</td><td>Raw value to subtract (tare)</td></tr><tr><td>Calibration Factor</td><td>Decimal</td><td>Raw value divided by this factor yields grams</td></tr><tr><td>Read Delay (ms)</td><td>Decimal</td><td>Delay between raw samples to improve stability</td></tr><tr><td>Max Retries</td><td>Integer</td><td>Retries for saturated or invalid samples</td></tr><tr><td>Clock Delay (µs)</td><td>Decimal</td><td>Delay per HX711 clock pulse</td></tr><tr><td>Ready Timeout (ms)</td><td>Decimal</td><td>Timeout waiting for HX711 ready</td></tr><tr><td>Filter Invalid Samples</td><td>Boolean</td><td>Discard saturated samples</td></tr><tr><td>Auto Gain Fallback</td><td>Boolean</td><td>Retry alternate gain when saturation occurs</td></tr><tr><td>Zero Tracking Enabled</td><td>Boolean</td><td>Slowly correct tare drift near zero load</td></tr><tr><td>Zero Tracking Threshold (g)</td><td>Decimal</td><td>Only adjust tare within this range</td></tr><tr><td>Zero Tracking Rate</td><td>Decimal</td><td>Fraction of offset corrected per measurement</td></tr></tbody></table>
...
Legacy GPIO input intended for Raspberry Pi 4 and earlier. For Raspberry Pi 5 and broad compatibility, use the CircuitPython input above.

<table><thead><tr class="header"><th>Option</th><th>Type</th><th>Description</th></tr></thead><tbody><tr><td>Measurements Enabled</td><td>Multi-Select</td><td>The measurements to record</td></tr><tr><td>Period (Seconds)</td><td>Decimal</td><td>The duration between measurements or actions</td></tr><tr><td>Pre Output</td><td>Select</td><td>Turn the selected output on before taking every measurement</td></tr><tr><td>Pre Out Duration (Seconds)</td><td>Decimal</td><td>If a Pre Output is selected, set the duration to turn the Pre Output on for before every measurement is acquired.</td></tr><tr><td>Pre During Measure</td><td>Boolean</td><td>Check to turn the output off after (opposed to before) the measurement is complete</td></tr><tr><td>Data Pin (DT)</td><td>Integer</td><td>The GPIO pin connected to the HX711 data pin (DT/DOUT)</td></tr><tr><td>Clock Pin (SCK)</td><td>Integer</td><td>The GPIO pin connected to the HX711 clock pin (SCK/PD_SCK)</td></tr><tr><td>Channel</td><td>Select</td><td>The HX711 channel to read from (A or B)</td></tr><tr><td>Gain</td><td>Select</td><td>Gain selection (A: 128/64, B: 32)</td></tr><tr><td>Samples</td><td>Integer</td><td>The number of samples to average per measurement</td></tr><tr><td>Outlier Filter Enabled</td><td>Boolean</td><td>Filter spikes using a median/MAD filter before averaging</td></tr><tr><td>Outlier MAD Threshold</td><td>Decimal</td><td>Lower is stricter; typical range 2.5–5.0</td></tr><tr><td>Tare Value</td><td>Decimal</td><td>Raw value to subtract (tare)</td></tr><tr><td>Calibration Factor</td><td>Decimal</td><td>Raw value divided by this factor yields grams</td></tr></tbody></table>
