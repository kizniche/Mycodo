/* MycoDo - Regulate temperature and humidity with raspberryPi and DHT22 sensor
   ...
   To write config: sudo mycodo w [minTemp] [maxTemp] [highTemp] [minHum] [maxHum] [webOR] [tempState] [humState]
 */
...
int RHums[] = {2, 4, 8, 10};
int RHumt[] = {0, 40, 40, 40, 40, 40};
int RHumo[] = {0, 90, 75, 60, 45, 30};
int RHumTS = 0;
...
int main( int argc, char *argv[] )
{
	...
	// read config file and print only variable values to screen
		...
		readCfg();
		printf("%i %i %i %i %i %i %i %i\n", minTemp, maxTemp, highTemp, minHum, maxHum, webOR, tempState, humState);
	}
		...
		readCfg();
		printf("\nminTemp: %i, maxTemp: %i, highTemp: %i, minHum: %i, maxHum: %i, webOR: %i, tempState: %i, humState: %i", minTemp, maxTemp, highTemp, minHum, maxHum, webOR, tempState, humState);
...
{
	if ( tdiff > RHeats[0] ) RFant[0] = 1;
	else if ( tdiff > RHeats[1] ) RFant[0] = 2;
	else if ( tdiff > RHeats[2] ) RFant[0] = 3;
	else if ( tdiff > RHeats[3] ) RFant[0] = 4;
	else RFant[0] = 5;

        if ( hdiff > RHums[0] ) RFant[0] = 1;
        else if ( hdiff > RHums[1] ) RFant[0] = 2;
        else if ( hdiff > RHums[2] ) RFant[0] = 3;
        else if ( hdiff > RHums[3] ) RFant[0] = 4;
        else RFant[0] = 5;
	...
	else if ( tempState == 1 && (humState == 2 || humState == 3) )
	{
		...
		// Relay 3 Circulatory Fan
		if (humState == 3) relay3(1); // Turn relay3 on
