...
char *cwd;
char *config_file = "/config/mycodo.conf";
...
int main(int argc, char *argv[]) {
	...
	config_file_path = concat(cwd, config_file);
	...
	if (strcmp(argv[1], "-w") == 0 || strcmp(argv[1], "--write") == 0)
		...
		printf("\n%s:%s:%s %i Read config file %s",  argv[4], argv[5], argv[6], timestampL, config_file_path);
		readCfg();
		...
		if (!webOR) {
			ChangeRelays ( maxTemp - temp, maxHum - hum );
		} else { 
...
int writeCfg(void) {
	...
	config_write_file(&cfg, config_file_path);
