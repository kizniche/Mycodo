...
def mycodo_database_create():
    ...
    conn = sqlite3.connect(sql_database_mycodo)
    cur = conn.cursor()
    cur.execute("INSERT OR IGNORE INTO CameraStill VALUES('0', 0, 1, 1, '', '', '-vf -hf -w 800 -h 600')")
    conn.commit()
    cur.close()
    ...
    conn = sqlite3.connect(sql_database_mycodo)
    cur = conn.cursor()
    cur.execute("INSERT OR IGNORE INTO CameraStream VALUES('0', 0, '', '', '--nopreview --contrast 20 --sharpness 60 --awb auto --quality 20 --vflip --hflip --width 800 --height 600')")
    conn.commit()
    cur.close()
