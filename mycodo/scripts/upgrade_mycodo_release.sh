...
runSelfUpgrade() {
  ...
  CURRENT_VERSION=$(${INSTALL_DIRECTORY}/env/bin/python ${INSTALL_DIRECTORY}/Mycodo/mycodo/utils/github_release_info.py -c 2>&1)
  BACKUP_DIR="/var/Mycodo-backups/Mycodo-backup-${NOW}-${CURRENT_VERSION}"
  UPDATE_VERSION=$(${INSTALL_DIRECTORY}/env/bin/python ${INSTALL_DIRECTORY}/Mycodo/mycodo/utils/github_release_info.py -m 5 -v 2>&1)
  MYCODO_NEW_TMP_DIR="/tmp/Mycodo-${UPDATE_VERSION}"
  UPDATE_URL=$(${INSTALL_DIRECTORY}/env/bin/python ${INSTALL_DIRECTORY}/Mycodo/mycodo/utils/github_release_info.py -m 5 2>&1)
  ...
  if [ $1 = "force-upgrade-master" ]; then
