...
async def execute_at_modification_async(
    ...
    if not device_id or device_id == custom_options_dict_presave["device_id"]:
        ...
        return messages, mod_input, custom_options_dict_postsave, custom_options_channels_dict_postsave
    ...
    if not node.interviewed:
        messages["error"].append(f"Node {node.uuid} not interviewed")
        return messages, mod_input, custom_options_dict_postsave, custom_options_channels_dict_postsave

    device_measurements = DeviceMeasurements.query.filter(DeviceMeasurements.device_id == mod_input.unique_id)
    channels = []
    ...
    for measurement in device_measurements.all():
        ...
        if endpoint and endpoint.ENDPOINT_CLASS == EndpointClass.Input and endpoint.INPUT_CLASS in mapping:
...
def execute_at_modification(
    ...
    return asyncio.run(execute_at_modification_async(
        messages,
        mod_input,
        request_form,
        custom_options_dict_presave,
...
class InputModule(AbstractInput):
    ...
    async def async_initialize(self):
        ...
        for measurement in self.device_measurements.all():
            ...
            self.measurement_info[measurement.channel]['measurement'] = measurement.measurement
