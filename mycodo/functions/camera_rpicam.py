# coding=utf-8
#
#  camera_rpicam.py - Capture image using rpicam library
#
#  Copyright (C) 2015-2025 Kyle T. Gabriel <mycodo@kylegabriel.com>
#
...
FUNCTION_INFORMATION = {
    'function_name_unique': 'CAMERA_RPICAM',
    'function_name': '{}: rpicam: {}/{}'.format(lazy_gettext("Camera"), lazy_gettext("Image"), lazy_gettext("Video")),
    'function_name_short': '{} rpicam'.format(lazy_gettext("Camera")),
    ...
    'message': 'NOTE: THIS IS CURRENTLY EXPERIMENTAL - USE AT YOUR OWN RISK UNTIL THIS NOTICE IS REMOVED. Capture images and videos from a camera using rpicam-still and rpicam-vid. The Function must be activated in order to capture still and timelapse images and use the Camera Widget.',
    ...
    'custom_options': [
        ...
        },
        {
            ...
            'default_value': '',
            'required': False,
            'name': "Custom rpicam-still Options",
            'phrase': "Pass custom options to the rpicam-still command."
        },
        {
            'type': 'message',
        ...
        },
        {
            ...
            'default_value': '',
            'required': False,
            'name': "Custom rpicam-vid Options",
            'phrase': "Pass custom options to the rpicam-vid command."
        },
    ]
}
    ...
    def capture(self, record_type, duration_sec=None, tmp_filename=None):
        try:
            ...
            if not os.path.exists("/usr/bin/rpicam-still"):
                self.logger.error("/usr/bin/rpicam-still not found")
                return None, None
            elif not os.path.exists("/usr/bin/rpicam-vid"):
                self.logger.error("/usr/bin/rpicam-vid not found")
                return None, None
