        ...
        try:
            if form_add_func.func_type.data == 'pid':
                ...
                if not error:
                    ...
                    db.session.commit()
            elif form_add_func.func_type.data == 'conditional':
                ...
                if not error:
                    ...
                    DisplayOrder.query.first().trigger = add_display_order(
                        display_order, new_func.unique_id)
                    db.session.commit()
...
def action_mod(form):
    ...
    try:
        ...
        if mod_action.action_type == 'output':
        ...
        if mod_action.action_type == 'output_pwm':
            mod_action.do_unique_id = form.do_unique_id.data
