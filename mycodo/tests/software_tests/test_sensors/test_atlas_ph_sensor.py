...
def test_atlas_ph_iterates_using_in():
    ...
    with mock.patch('mycodo.sensors.atlas_ph.AtlaspHSensor.get_measurement') as mock_measure:
        mock_measure.side_effect = [67, 52, 37, 45]  # first reading, second reading

        atlas_ph = AtlaspHSensor('I2C', testing=True)
...
def test_atlas_ph_raises_exception():
    ...
    with mock.patch('mycodo.sensors.atlas_ph.AtlaspHSensor.get_measurement', side_effect=IOError):
        with pytest.raises(StopIteration):
            AtlaspHSensor('I2C', testing=True).next()


def test_atlas_ph_read_returns_1_on_exception():
    """ Verify the read() method returns true on error """
    with mock.patch('mycodo.sensors.atlas_ph.AtlaspHSensor.get_measurement', side_effect=Exception):
        assert AtlaspHSensor('I2C', testing=True).read()


# def test_atlas_ph_get_measurement_divs_by_1k():
#     """ verify the return value of get_measurement """
#     mocked_open = mock.mock_open(read_data='45780')  # value read from sys ph file
#     with mock.patch('mycodo.sensors.atlas_ph.open', mocked_open, create=True):
#         assert AtlaspHSensor.get_measurement() == 45.78  # value read / 1000

def test_atlas_ph_read_logs_unknown_errors():
    """ verify that IOErrors are logged """
    with LogCapture() as log_cap:
        # force an Exception to be raised when get_measurement is called
        with mock.patch('mycodo.sensors.atlas_ph.AtlaspHSensor.get_measurement', side_effect=Exception('msg')):
            AtlaspHSensor('I2C', testing=True).read()
    expected_logs = ('mycodo.sensors.atlas_ph', 'ERROR', 'AtlaspHSensor raised an exception when taking a reading: msg')
