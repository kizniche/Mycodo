...
def test_tsl2591_sensor_raises_exception():
    ...
    with mock.patch('mycodo.inputs.tsl2591_sensor.TSL2591Sensor.get_measurement', side_effect=IOError):
        with pytest.raises(StopIteration):
            TSL2591Sensor(None, None, testing=True).next()


def test_tsl2591_sensor_read_returns_1_on_exception():
    """ Verify the read() method returns true on error """
    with mock.patch('mycodo.inputs.tsl2591_sensor.TSL2591Sensor.get_measurement', side_effect=Exception):
        assert TSL2591Sensor(None, None, testing=True).read()
