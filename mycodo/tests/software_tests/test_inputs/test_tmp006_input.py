...
def test_tmp006_iterates_using_in():
    ...
    with mock.patch('mycodo.inputs.tmp006.InputModule.get_measurement') as mock_measure:
...
def test_tmp006_special_method_repr():
    ...
    with mock.patch('mycodo.inputs.tmp006.InputModule.get_measurement') as mock_measure:
        ...
        tmp006 = TMP006Sensor(None, testing=True)
        tmp006.read()
        assert "<InputModule(temperature_die=0.00)(temperature_object=0.00)>" in repr(tmp006)


def test_tmp006_raises_exception():
    """ stops iteration on read() error """
    with mock.patch('mycodo.inputs.tmp006.InputModule.get_measurement', side_effect=IOError):
...
def test_tmp006_read_returns_1_on_exception():
    ...
    with mock.patch('mycodo.inputs.tmp006.InputModule.get_measurement', side_effect=Exception):
...
def test_tmp006_read_logs_unknown_errors():
    ...
    with LogCapture() as log_cap:
        with mock.patch('mycodo.inputs.tmp006.InputModule.get_measurement',
                        side_effect=Exception('msg')):
            TMP006Sensor(None, testing=True).read()
    expected_logs = ('mycodo.inputs.tmp006', 'ERROR', 'InputModule raised an exception when taking a reading: msg')
