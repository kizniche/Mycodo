...
def test_atlas_pt1000_iterates_using_in():
    ...
    with mock.patch('mycodo.inputs.atlas_pt1000.InputModule.get_measurement') as mock_measure:
...
def test_atlas_pt1000_special_method_repr():
    ...
    with mock.patch('mycodo.inputs.atlas_pt1000.InputModule.get_measurement') as mock_measure:
        ...
        atlas_pt1000 = AtlasPT1000Sensor(None, testing=True)
        atlas_pt1000.read()
        assert "<InputModule(temperature=0.00)>" in repr(atlas_pt1000)


def test_atlas_pt1000_raises_exception():
    """ stops iteration on read() error """
    with mock.patch('mycodo.inputs.atlas_pt1000.InputModule.get_measurement', side_effect=IOError):
...
def test_atlas_pt1000_read_returns_1_on_exception():
    ...
    with mock.patch('mycodo.inputs.atlas_pt1000.InputModule.get_measurement', side_effect=Exception):
...
def test_atlas_pt1000_read_logs_unknown_errors():
    ...
    with LogCapture() as log_cap:
        with mock.patch('mycodo.inputs.atlas_pt1000.InputModule.get_measurement', side_effect=Exception('msg')):
            AtlasPT1000Sensor(None, testing=True).read()
    expected_logs = ('mycodo.inputs.atlas_pt1000', 'ERROR', 'InputModule raised an exception when taking a reading: msg')
