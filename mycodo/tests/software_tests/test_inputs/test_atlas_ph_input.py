...
def test_atlas_ph_iterates_using_in():
    ...
    with mock.patch('mycodo.inputs.atlas_ph.InputModule.get_measurement') as mock_measure:
...
def test_atlas_ph_special_method_repr():
    ...
    with mock.patch('mycodo.inputs.atlas_ph.InputModule.get_measurement') as mock_measure:
        ...
        atlas_ph = AtlaspHSensor(None, testing=True)
        atlas_ph.read()
        assert "<InputModule(ph=0.00)>" in repr(atlas_ph)


def test_atlas_ph_raises_exception():
    """ stops iteration on read() error """
    with mock.patch('mycodo.inputs.atlas_ph.InputModule.get_measurement', side_effect=IOError):
...
def test_atlas_ph_read_returns_1_on_exception():
    ...
    with mock.patch('mycodo.inputs.atlas_ph.InputModule.get_measurement', side_effect=Exception):
...
def test_atlas_ph_read_logs_unknown_errors():
    ...
    with LogCapture() as log_cap:
        with mock.patch('mycodo.inputs.atlas_ph.InputModule.get_measurement', side_effect=Exception('msg')):
            AtlaspHSensor(None, testing=True).read()
    expected_logs = ('mycodo.inputs.atlas_ph', 'ERROR', 'InputModule raised an exception when taking a reading: msg')
