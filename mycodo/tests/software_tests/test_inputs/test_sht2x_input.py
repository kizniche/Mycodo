...
def test_sht2x_read_updates_temp():
    ...
    with mock.patch('mycodo.inputs.sht2x.SHT2xSensor.get_measurement') as mock_measure:
        ...
        mock_measure.side_effect = [(23, 50, 3000),
                                    (25, 55, 3200)]
        sht2x = SHT2xSensor(None, None, testing=True)
        assert sht2x._dew_point is None  # initial values
        assert sht2x._humidity is None
        assert sht2x._temperature is None
        assert not sht2x.read()  # updating the value using our mock_measure side effect has no error
        assert sht2x._dew_point == 23.0  # first values
        assert sht2x._humidity == 50.0
        assert sht2x._temperature == 3000.0
        assert not sht2x.read()  # updating the value using our mock_measure side effect has no error
        assert sht2x._dew_point == 25.0  # second values
...
def test_sht2x_condition_properties():
    ...
    with mock.patch('mycodo.inputs.sht2x.SHT2xSensor.get_measurement') as mock_measure:
        ...
        assert sht2x._humidity is None
        assert sht2x._temperature is None
        assert sht2x.dew_point == 23.0  # first reading with auto update
        assert sht2x.dew_point == 23.0  # same first reading, not updated yet
        assert sht2x.humidity == 50.0
        assert sht2x.humidity == 50.0
        assert sht2x.temperature == 3000.0
        assert sht2x.temperature == 3000.0
        assert not sht2x.read()  # update (no errors)
        assert sht2x.dew_point == 25.0  # next readings
