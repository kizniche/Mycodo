...
def test_sht2x_iterates_using_in():
    ...
    with mock.patch('mycodo.inputs.sht2x.InputModule.get_measurement') as mock_measure:
...
def test_sht2x_special_method_repr():
    ...
    with mock.patch('mycodo.inputs.sht2x.InputModule.get_measurement') as mock_measure:
        ...
        sht2x = SHT2xSensor(None, testing=True)
        sht2x.read()
        assert "<InputModule(dewpoint=0.00)(humidity=0.00)(temperature=0.00)>" in repr(sht2x)


def test_sht2x_raises_exception():
    """ stops iteration on read() error """
    with mock.patch('mycodo.inputs.sht2x.InputModule.get_measurement', side_effect=IOError):
...
def test_sht2x_read_returns_1_on_exception():
    ...
    with mock.patch('mycodo.inputs.sht2x.InputModule.get_measurement', side_effect=Exception):
...
def test_sht2x_read_logs_unknown_errors():
    ...
    with LogCapture() as log_cap:
        with mock.patch('mycodo.inputs.sht2x.InputModule.get_measurement',
                        side_effect=Exception('msg')):
            SHT2xSensor(None, testing=True).read()
    expected_logs = ('mycodo.inputs.sht2x', 'ERROR', 'InputModule raised an exception when taking a reading: msg')
