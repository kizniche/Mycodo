...
def test_mh_z19_iterates_using_in():
    ...
    with mock.patch('mycodo.inputs.mh_z19.InputModule.get_measurement') as mock_measure:
...
def test_mh_z19_special_method_repr():
    ...
    with mock.patch('mycodo.inputs.mh_z19.InputModule.get_measurement') as mock_measure:
        ...
        mh_z19 = MHZ19Sensor(None, testing=True)
        mh_z19.read()
        assert "<InputModule(co2=0.00)>" in repr(mh_z19)


def test_mh_z19_raises_exception():
    """ stops iteration on read() error """
    with mock.patch('mycodo.inputs.mh_z19.InputModule.get_measurement', side_effect=IOError):
...
def test_mh_z19_read_returns_1_on_exception():
    ...
    with mock.patch('mycodo.inputs.mh_z19.InputModule.get_measurement', side_effect=Exception):
...
def test_mh_z19_read_logs_unknown_errors():
    ...
    with LogCapture() as log_cap:
        with mock.patch('mycodo.inputs.mh_z19.InputModule.get_measurement', side_effect=Exception('msg')):
            MHZ19Sensor(None, testing=True).read()
    expected_logs = ('mycodo.inputs.mh_z19', 'ERROR', 'InputModule raised an exception when taking a reading: msg')
