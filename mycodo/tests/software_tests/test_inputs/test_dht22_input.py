...
def test_dht22_read_updates_temp():
    ...
    with mock.patch('mycodo.inputs.dht22.DHT22Sensor.get_measurement') as mock_measure:
        ...
        assert not dht22.read()  # updating the value using our mock_measure side effect has no error
        assert dht22._dew_point == 23.0  # first values
        assert dht22._humidity == 50.0
        assert dht22._temperature == 3000.0
        assert not dht22.read()  # updating the value using our mock_measure side effect has no error
        assert dht22._dew_point == 25.0  # second values
...
def test_dht22_condition_properties():
    ...
    with mock.patch('mycodo.inputs.dht22.DHT22Sensor.get_measurement') as mock_measure:
        ...
        assert dht22._humidity is None
        assert dht22._temperature is None
        assert dht22.dew_point == 23.0  # first reading with auto update
        assert dht22.dew_point == 23.0  # same first reading, not updated yet
        assert dht22.humidity == 50.0
        assert dht22.humidity == 50.0
        assert dht22.temperature == 3000.0
        assert dht22.temperature == 3000.0
        assert not dht22.read()  # update (no errors)
        assert dht22.dew_point == 25.0  # next readings
